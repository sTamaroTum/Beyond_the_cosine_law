...
class FlorisInterface(LoggerBase):
    ...
    def get_turbine_powers(self) -> NDArrayFloat:
        ...
        turbine_powers = power(
                velocitiesINPUT=self.floris.flow_field.u,
                yaw_angle=self.floris.farm.yaw_angles,
                cl_alfa=4.795594517062730,
                cd=0.003990283037936,
                beta=-3.177385547238738,
                theta_in=self.floris.farm.theta,
                sigma=0.0416,
                R=65,
                delta=-5,
                tsr=self.floris.farm.tsr ,
                rho=self.floris.flow_field.air_density,
                shear=self.floris.flow_field.wind_shear
                )
    ...
    def get_turbine_Cts(self) -> NDArrayFloat:
        turbine_Cts = Ct(
                ...
                cd=0.003990283037936,
                beta=-3.177385547238738,
                k=self.floris.flow_field.wind_shear,
                theta_in=self.floris.farm.theta,
                sigma=0.0416,
                R=65,
                delta=-5,
