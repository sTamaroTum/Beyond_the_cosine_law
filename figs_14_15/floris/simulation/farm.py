...
class Farm(BaseClass):
    ...
    theta_sorted: NDArrayFloat = field(init=False, default=[])
    tsr_sorted: NDArrayFloat = field(init=False, default=[])
    ...
    def construct_theta(self):
        self.theta = np.array([
            turb['theta'] for turb in self.turbine_definitions
        ])

    def construct_tsr(self):
        self.tsr = np.array([
            turb['tsr'] for turb in self.turbine_definitions
        ])
    ...
    def expand_farm_properties(
        ...
        self.theta_sorted = np.take_along_axis(
            self.theta * template_shape,
            sorted_coord_indices,
            axis=2
        )
        self.tsr_sorted = np.take_along_axis(
            self.tsr * template_shape,
            sorted_coord_indices,
            axis=2
        )
    ...
    def finalize(self, unsorted_indices):
        ...
        self.tsr = np.take_along_axis(
            self.tsr_sorted,
            unsorted_indices[:,:,:,0,0],
            axis=2
        )
        self.theta = np.take_along_axis(
            self.theta_sorted,
            unsorted_indices[:,:,:,0,0],
            axis=2
        )
