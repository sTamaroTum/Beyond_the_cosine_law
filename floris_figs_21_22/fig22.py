### Optimal wind farm control of a small cluster of two wind turbines

## This is an interactive version of Figure 22 found in Tamaro et al (in review). This code calles the modified version of FLORIS included in this same folder,
...
plt.subplot(2,3,1)
plt.plot(wind_directions_plot_FLORIS,optimal_tsr[idxF],'-',color='deepskyblue',label='Floris opt.',linewidth=1.75)
plt.plot(wind_directions_plot,x_trust_constr[idx,0],'--',color='lightcoral',label='Model',linewidth=1.75)
plt.plot(np.array([240,300]),np.array([8.20949995,8.20949995]),':C2',linewidth=1.75)
plt.ylabel(r'$\lambda$ [-]')
plt.ylim([7.5,8.5])
plt.subplot(2,3,2)
plt.plot(np.array([240,300]),np.array([1.08500285,1.08500285]),':C2',label='Greedy',linewidth=1.75)
plt.plot(wind_directions_plot_FLORIS,optimal_pitch[idxF],'-',color='deepskyblue',label='Optimal based on $\cos^{p_p}$ law',linewidth=1.75)
plt.plot(wind_directions_plot,x_trust_constr[idx,1],'--',color='lightcoral',label='Optimal based on present model',linewidth=1.75)
plt.ylabel(r'$\theta_p$ [$^\circ$]')
plt.ylim([-1,1.5])
plt.legend(bbox_to_anchor=(2,1.45),ncol=3,frameon=False)
plt.subplot(2,3,3)
plt.plot(wind_directions_plot_FLORIS,np.abs(floris_gamma_o[idxF]),'-',color='deepskyblue',label='Floris opt.',linewidth=1.75)
plt.plot(wind_directions_plot,np.abs(x_trust_constr[idx,2]),'--',color='lightcoral',label='Model',linewidth=1.75)
plt.plot(np.array([240,300]),np.array([0,0]),':C2',linewidth=1.75)
plt.ylabel(r'$|\gamma|$ [$^\circ$]')
plt.ylim([-5,30])
plt.subplot(2,3,6)
plt.plot(wind_directions_plot_FLORIS,((np.sum(optimum_power_floris[idxF,:],axis=1)/np.sum(pow_greedy_floris[idxF,:],axis=1))-1)*100,'-',color='deepskyblue',label='Floris opt.',linewidth=1.75)
podenza = np.interp(wind_directions_plot_FLORIS,wind_directions_plot,((max_trust_constr[idx])))
plt.plot(wind_directions_plot_FLORIS,((podenza/np.sum(pow_greedy_floris[idxF,:],axis=1))-1)*100,'--',color='lightcoral',label='Model',linewidth=1.75)
plt.plot(np.array([240,300]),np.array([0,0]),'C2:',zorder=-1,linewidth=1.75)
plt.ylabel(r'$\Delta P_{\textrm{TOT}}$ [\%]')
plt.xlabel(r'$\Phi$ [$^\circ$]')
plt.subplot(2,3,4)
plt.plot(wind_directions_plot_FLORIS,((optimum_power_floris[idxF,0]/(pow_greedy_floris[idxF,0]))-1)*100,'-',color='deepskyblue',linewidth=1.75)#,label='FLORIS (WT1)')
podenza = np.interp(wind_directions_plot_FLORIS,wind_directions_plot,((i_pow[idx,0])))
plt.plot(wind_directions_plot_FLORIS,((podenza/(pow_greedy_floris[idxF,0]))-1)*100,'--',color='lightcoral',linewidth=1.75)
plt.plot(np.array([240,300]),np.array([0,0]),'C2:',zorder=-1,linewidth=1.75)
plt.xlim([240,300])
...
plt.xlabel(r'$\Phi$ [$^\circ$]')
plt.subplot(2,3,5)
plt.plot(wind_directions_plot_FLORIS,((optimum_power_floris[idxF,1]/(pow_greedy_floris[idxF,1]))-1)*100,'-',color='deepskyblue',linewidth=1.75)
podenza = np.interp(wind_directions_plot_FLORIS,wind_directions_plot,((i_pow[idx,1])))
plt.plot(wind_directions_plot_FLORIS,((podenza/(pow_greedy_floris[idxF,1]))-1)*100,'--',color='lightcoral',linewidth=1.75)
plt.plot(np.array([240,300]),np.array([0,0]),'C2:',zorder=-1,linewidth=1.75)
plt.xlim([240,300])
plt.plot(np.array([200,210]),np.array([0,0]),'-k',label='WT1')
plt.plot(np.array([200,210]),np.array([0,0]),'k--',label='WT2')
...
## Add to subplots
plt.subplot(2,3,1)
plt.scatter(phi2,np.array([7.971624176830841, 7.827879563749586, 7.913594812836695,7.837035641612392,7.976773533259332]),color='deepskyblue',label='floris',marker='o')
plt.scatter(phi2,np.array([8.124163592825822,8.030666241654806,8.01227337846487,8.039962163186885,8.132238856257297]),color='lightcoral',label='floris',marker='o')
plt.subplot(2,3,2)
plt.scatter(phi2,np.array([1.08500285,1.08500285,1.08500285,1.08500285,1.08500285]),color='deepskyblue',label='floris',marker='o')
plt.scatter(phi2,np.array([0.8026487336572626,0.2887554470923204,0.3839288903095631,0.3207476002821303,0.8367043355324543]),color='lightcoral',label='floris',marker='o')
plt.subplot(2,3,3)
plt.scatter(phi2,np.abs(np.array([-17.41470904098508,-22.049588305395208,-19.417331380641855,21.783805647385744,17.220567756031347])),color='deepskyblue',label='floris',marker='o')
plt.scatter(phi2,np.abs(np.array([-20.193898143602926,-26.404169711739435,-26.4907131832505,25.942086790841064,19.92084003228294])),color='lightcoral',label='floris',marker='o')
plt.subplot(2,3,4)
plt.scatter(phi2,(POW[:,1]/POW[2,0]-1)*100,color='deepskyblue',label='floris')
plt.scatter(phi2,(POW[:,2]/POW[2,0]-1)*100,color='lightcoral',label='model')
plt.subplot(2,3,5)
plt.scatter(phi2,(ppp[1,:]/ppp[0,:]-1)*100,color='deepskyblue',label='floris',marker='o')
plt.scatter(phi2,(ppp[2,:]/ppp[0,:]-1)*100,color='lightcoral',label='model',marker='o')
plt.subplot(2,3,6)
plt.scatter(phi2,((ppp[1,:]+POW[:,1]/1)/(ppp[0,:]+POW[2,0]/1)-1)*100,color='deepskyblue',label='floris')
plt.scatter(phi2,((ppp[2,:]+POW[:,2]/1)/(ppp[0,:]+POW[2,0]/1)-1)*100,color='lightcoral',label='model')
for i in np.arange(6):
    plt.subplot(2,3,i+1)
    plt.xlim([245,295])
    if i < 3:
