...
def sequential_solver(
    ...
    for i in range(grid.n_turbines):
        ...
        tsr_i = farm.tsr_sorted[: ,:, i:i+1, None, None]
        theta_i = farm.theta_sorted[: ,:, i:i+1, None, None]
        
        ct_i = Ct(
            velocitiesINPUT=flow_field.u_sorted,
            yaw_angle=farm.yaw_angles_sorted,
            cl_alfa=4.795594517062730,
            cd=0.003990283037936,
            beta=-3.177385547238738,
            k=flow_field.wind_shear,
            theta_in=theta_i,
            sigma=0.0416,
            R=65,
            delta=-5,
...
def full_flow_sequential_solver(
    ...
    #turbine_grid_farm,construct_theta()
    #turbine_grid_farm,construct_tsr()
