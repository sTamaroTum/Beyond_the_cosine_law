### Optimal control of a yawed wind turbine

## This is an interactive version of Figure 21 found in Tamaro et al (in review). This code calles the modified version of FLORIS included in this same folder,
...
for yaw in gamma_array:    
    ...
    res = minimize(optimization_f, x0,args=(yaw,wDirection,nD),bounds=bnds,method='Nelder-Mead',options={'maxiter':80000})
    print(res.x)
...
plt.subplot(2,2,1)
plt.plot(gamma_array2,tsr_controllo_normale[idx],'-',color='deepskyblue',label='Standard')
plt.plot(gamma_array2,x_trust_constr[idx,0],'--',color='lightcoral',label='Optimal')
plt.ylabel(r'$\lambda$ [-]')
plt.legend(ncol=2,frameon=False,bbox_to_anchor=(1.7,1.6))
plt.ylim([7.5,8.5])
...
plt.text(24,8.3,'(a)')
plt.subplot(2,2,2)
plt.plot(gamma_array2,pitch_controllo_normale[idx],'-',color='deepskyblue',label='Model')
plt.plot(gamma_array2,x_trust_constr[idx,1],'--',color='lightcoral',label='Model')
...
plt.text(23,1.2,'(b)')
plt.subplot(2,2,3)
plt.plot(gamma_array2,(i_ct[idx,0]/(i_ct_controllo_normale[idx])-1)*100,':k',label='Model')
...
plt.text(24,16,'(c)')
plt.subplot(2,2,4)
plt.plot(gamma_array2,(i_pow[idx,0]/(-wf_pow_controllo_normale[idx])-1)*100,':k',label='Model')
plt.ylim([0,4])
plt.grid()
