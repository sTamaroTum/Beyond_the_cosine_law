...
#%%
print('meshgridding')
[X,Y,Z] = np.meshgrid(np.arange(-110,1040,2), np.arange(-100,141,1), np.arange(0,281));
# [X,Y,Z] = np.meshgrid(np.arange(-110,1040,2), np.arange(-80,141,5), np.arange(0,281,2));

#%%
print('griddataing')
from scipy.interpolate import griddata
vq = griddata((z,x,y),Q,(Z,X,Y),'linear');
# uq = griddata((x,y,z),U,(X,Y,Z),'nearest');
#%%
# xxx = np.linspace(-100,1040,2)
...
plot.get_snapshot()
with open('Figure4b.html','w') as fp:
